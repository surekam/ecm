...
const char *SUREKAMEncodingCStringFromNSStringEncoding(NSStringEncoding encoding)
{
...
- (id)initWithData:(NSData *)data options:(NSUInteger)mask error:(NSError **)error
{
	...
	const char *xmlEncoding = SUREKAMEncodingCStringFromNSStringEncoding(NSUTF8StringEncoding);
    
	//xmlDocPtr doc = xmlParseMemory([data bytes], [data length]);
    xmlDocPtr doc = xmlReadMemory((const char*)[data bytes],
                                  (int)[data length], baseURL,
                                  xmlEncoding,
